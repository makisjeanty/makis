...
- `cursos/` — newest app: interactive-lesson platform, no login. `Curso` (nivel iniciante/intermediario/avancado, `icone` emoji, `ativo` flag) → `Modulo` → `Licao` (`duracao_minutos`) → `Etapa` (`tipo` slide/quiz/code, quiz options in `opcoes_json`), each level ordered by `ordem`. `Curso.save()` auto-slugifies `titulo`. Admin nests `TabularInline`s down the whole hierarchy. Views: `lista_cursos`, `trilha_curso` (`prefetch_related('licoes')`), `executar_licao` — no progress tracking/persistence yet. Has `tests.py` (models + views, active-only filtering, 404s)
...
- `/cursos/`, `/cursos/<slug>/`, `/cursos/licao/<int:licao_id>/` → `cursos.urls` (namespace `cursos`) — course list, trilha (modules/lessons), single-lesson view
...
## Data-seeding scripts (repo root)
- `popular_dados.py`, `popular_cursos.py`, `adicionar_artigos_blog.py`, `adicionar_produtos_anuncio.py` — standalone one-off content scripts (portfolio projects, blog posts, `cursos` trees, product-announcement portfolio entries), not management commands. Each bootstraps Django itself (`os.environ.setdefault('DJANGO_SETTINGS_MODULE', 'core.settings')` + `django.setup()`) and runs directly: `python popular_dados.py`. Most use `get_or_create`/slug lookups so re-running is usually safe, but not universally — check each script before re-running against a populated DB.

## Non-code documentation (repo root)
- `business/` — product/business planning docs (`VISION.md`, `ROADMAP.md`, `MONETIZATION.md`, `MARKET_RESEARCH.md`, `PRODUCT_IDEAS.md`, `ASSETS.md`, `CONTENT_PIPELINE.md`), not consumed at runtime. `docs/adr/` holds architecture decision records (e.g. `ADR-BUSINESS-001.md`). `RUNBOOK.md`, `GO_LIVE_CHECKLIST.md`, `AUDITORIA_SEGURANCA.md`, `ajuste.env.md` are operational docs for the real deploy — read them before touching prod-facing config rather than re-deriving that context from code.

## Deployment (Docker)
- Production target: Contabo VPS (Ubuntu 24.04) behind Cloudflare in **Full (strict)** SSL mode, domain `makisjeanty.com` — see `GO_LIVE_CHECKLIST.md`/`RUNBOOK.md` for actual DNS/TLS/backup/incident procedures.
...
- `portfolio`, `blog`, `accounts`, `utilidades`, `comunidade`, `chat`, and `cursos` all have views/urls/templates implemented, including profile pages, a moderated comment form, 4 client-side tools, an open forum, real-time chat, and interactive lessons
